...
#if !defined(ONE_GLOBAL_SECTION) && !defined(NO_INIT)
...
#if defined(NO_INIT)
#define __libfpinit(x) 0
#define __libfpterm(x)
#endif
                     ...
                     register __a6 struct MyLibrary *libbase )
{
...
#ifdef GLOBAL_PARTS    
    if (!libbase->ml_origbase->ml_global)
    {
        libbase->ml_origbase->ml_global =
            AllocMem(sizeof(globalPart),MEMF_CLEAR);
        
        if (!libbase->ml_origbase->ml_global) goto error;
        
        __SetLibGlobals(libbase, libbase->ml_origbase->ml_global);
        __InitLibGlobals(libbase);
    }
    else
    {
        __SetLibGlobals(libbase,libbase->ml_origbase->ml_global);
       ...
       )
    {
       ...
       FreeMem(newlib, (long)(MYLIBRARYSIZE + 
                       ((long)&RESLEN) + SIZEJMPTAB));
error:
       libbase->ml_origbase->ml_Lib.lib_OpenCnt--;
       return NULL; /* abort if user init failed */
    }
...
ULONG __asm _LibExpunge( register __a6 struct MyLibrary *libbase )
{
    ...
    if (libbase->ml_Lib.lib_OpenCnt == 0)
    {
        ...
        __SetLibGlobals(libbase,libbase->ml_global);
        
        if (libbase->ml_global)
        {
            __CleanupLibGlobals(libbase);
            FreeMem(libbase->ml_global, sizeof *libbase->ml_global);
            libbase->ml_global = 0;
        }
    ...
    {
        /* Let the user free any unnecessary resources */
        __UserLibExpunge(libbase);
    }
