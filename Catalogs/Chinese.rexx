...
'C:Makedir '||datadir||' '||datadir||'/Hanzi '||datadir||'/Vocabulary '
Address
/**/
'Echo' 'Processing ...'
/**/
...
If Open(DBFH,ucodedata,READ) Then Do
	Do While ~Eof(DBFH)
		...
		If SubStr(L,1,2)='U+' Then Do
			...
			Select
				When dbEntryType='kMandarin' Then WriteOutputEntries('M='||Vector);
				When dbEntryType='kCantonese' Then WriteOutputEntries('M='||Vector);
				OtherWise NOP;
			End;
		End;
        If MF=11 Then Do;
...
WriteOutputEntries: PROCEDURE EXPOSE datadir
	Options Results
	Parse Arg Variant Codepoint Ideograph Reading
/**/
